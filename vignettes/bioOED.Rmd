---
title: "bioOED: Optimum Experient Design for Microbial Inactivation"
...
The R-package __bioOED__ implements functions for the design of optimum experiments, as well as for the calculation of local sensitivities, of microbil inactivation processes. This package can be downloaded form the Comprehensive R Archive Network (CRAN). Once installed, it can be loaded as
...
* *inactivation_OED()*: Designs an optimum experiment based onn a given measurement of the FIM.
...
The FIM is relevant for the OED because its inverse is an estimator of the covariance matrix of the error. Hence, an OED can be made by optimizing certain measurements of the FIM. __bioOED__ implements two of this criteria: D and modified E. The D-criterium consists on the maximization of the determinant of the FIM. It is equivalent to the minimization of the volume of the confidence elipsoids. The modified E criterium is based on the minimization of the ratio between the maximum and minimum eigen value of the FIM ($\mathrm{abs} \frac{\lambda_{max}}{\lambda_{min}}$). 
...
Typically, this derivative cannot be calculated in close form and one has to use numeric methods. __bioOED__ uses the function *sensFun()* from the __FME__ package for this purpuose, which approximates the local sensitivity functions using finite diffences:
$$
    S_{ij}(t) \approx
...
The argument `sensvar` allows chosing whether the sensitivity of the microbial count ($N$) or its decimal logarithm ($\log N$) with respect to the model parameters will be calculated. By default, the sensitivity of $\log N$ is calculated.
...
The __bioOED__ package implements the function *calculate_pars_correlation()* which calculates the correlation betweent the sensitivity function of the model, providing an indicator of its structural identificability. This function has 6 input arguments:
...
*calculate_pars_correlation()* provides a matrix containing the correlation between the sensitivity functions of the model parameters considered.
...
The criteria for the OED is defined by the argument `criteria`. It must be a characer with the value `"D"` or `"E-mod"`. By default, the D-criterium is set.
...
global_OED <- inactivation_OED(inactivation_model, parms, temp_profile,
                               parms_fix, n_points, criteria = "E-mod",
...
* `penalty`: logical indicating wheter a penalty function has been used.
...
http://CRAN.R-project.org/package=bioinactivation
